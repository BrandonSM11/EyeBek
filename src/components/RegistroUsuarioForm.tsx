...
export default function RegistroUsuarioForm({ userType = 'coder' }: { userType?: string }) {
      ...
      if (videoRef.current) {
        ...
        videoRef.current.onloadedmetadata = () => {
          if (canvasRef.current) {
            ...
            console.log('📹 Canvas configurado:', canvasRef.current.width, 'x', canvasRef.current.height);
          }
        };
        setShowCamera(true);
      }
  ...
  const capturePhoto = () => {
    if (canvasRef.current && videoRef.current) {
      ...
      if (context) {
        ...
        // Dibujar el video en el canvas con las dimensiones correctas
        context.drawImage(videoRef.current, 0, 0, canvasRef.current.width, canvasRef.current.height);
        const imageData = canvasRef.current.toDataURL('image/jpeg', 0.9);
        console.log('✅ Foto capturada, tamaño:', imageData.length);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrors({ ...errors, email: data.message || 'Error al registrar' });
      }
    } catch (error) {
    ...
    <div className="bg-white rounded-xl shadow-lg p-8 max-w-2xl mx-auto">
      ...
      {success && (
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <p className="text-green-700 font-semibold">✓ Usuario registrado exitosamente</p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="pt-4">
          <GenericButton
            textButton={loading ? 'Registrando...' : 'Registrar Usuario'}
            type="submit"
            variant="black"
            size="full"
            disabled={loading}
